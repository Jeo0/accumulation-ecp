...
namespace Activity5
{
    public partial class Activity5 : Form
    {
        ...
        {
            double basic_numhrs, basic_rate, basic_netincome;
            if (!double.TryParse(basic_numhrsTxtbox.Text, out basic_numhrs))
            {
            ...
            // Try to parse basic_rate
            if (!double.TryParse(basic_rateTxtbox.Text, out basic_rate))
            {
                basic_rate = 0; // Default value if parsing fails
            }
        ...
        {
            double hono_numhrs, hono_rate, hono_netincome;

            // Try to parse hono_numhrs
            if (!double.TryParse(hono_numhrsTxtbox.Text, out hono_numhrs))
            {
                hono_numhrs = 0; // Default to 0 if parsing fails
            }

            // Try to parse hono_rate
            if (!double.TryParse(hono_rateTxtbox.Text, out hono_rate))
            {
                hono_rate = 0; // Default to 0 if parsing fails
            }
        ...
        private void printPayslip_btn_Click(object sender, EventArgs e)
        {
            ...
            Activity5_PrintFrms print1 = new Activity5_PrintFrms();
